...
def explode(depth, values):
    if 4 in depth:
        ...
        if idx == 0:
            ...
            depth = [depth[2]]*2 + depth[3:]
        elif idx == len(depth) -2:
            values = values[:idx-1] + [values[idx-1] + values[idx], 0]
            depth = depth[:idx-1] + [depth[idx-1]] * 2
        else:
            values = values[:idx-1] + [values[idx-1] + values[idx], 0, values[idx+1] + values[idx+2]] + values[idx+3:]
...
# [[[[4,2],2],6],[8,7]]""",]
    ...
    # for test in tests:
        ...
        d, v = add(*parse("[[[0,[4,5]],[0,0]],[[[4,5],[2,6]],[9,5]]]"), 
            *parse("[7,[[[3,7],[4,3]],[[6,3],[8,8]]]]"))
        d, v = reduce(d, v)
        print(d, v)
        d, v = add(*parse("[[[[4,0],[5,4]],[[7,7],[6,0]]],[[8,[7,7]],[[7,9],[5,0]]]]"), *parse("[[2,[[0,8],[3,4]]],[[[6,7],1],[7,[1,6]]]]"))
        d, v = reduce(d, v)
        print(d, v)